  ...
  useEffect(() => {
    const checkSession = async () => {
      ...
      if (isSupabasePlaceholder) {
        console.info("Using placeholder Supabase. Checking Guest Onboarding.");
        const localOnboarding = localStorage.getItem("guest_onboarding_completed");
        if (localOnboarding === "true" || !requireOnboarding) {
          setStatus("in");
        } else {
    ...
    const timeoutId = setTimeout(() => {
      setStatus((prev) => prev === "loading" ? "out" : prev);
    }, 2000);
