...
export function useConversations(userId: string | null) {
  ...
  useEffect(() => {
    ...
    if (!userId || isSupabasePlaceholder) {
      if (isSupabasePlaceholder && userId) console.info("Running in Guest Mode: Skipping DB load.");
      setConversations([]);
      return;
    }
    let cancelled = false;
  ...
  const ensureConversation = useCallback(
    async (convId: string, title: string): Promise<string | null> => {
      const isSupabasePlaceholder = import.meta.env.VITE_SUPABASE_URL?.includes("placeholder") || !import.meta.env.VITE_SUPABASE_URL;
      if (!userId || isSupabasePlaceholder) return convId;
      return null;
    },
    [userId]
  );
  ...
  const saveMessage = useCallback(
    async (conversationId: string, role: "user" | "assistant", content: string, id?: string) => {
      const isSupabasePlaceholder = import.meta.env.VITE_SUPABASE_URL?.includes("placeholder") || !import.meta.env.VITE_SUPABASE_URL;
      if (!userId || isSupabasePlaceholder) return;
    },
    [userId]
  );
